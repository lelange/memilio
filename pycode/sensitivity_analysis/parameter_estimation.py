...
df=pd.read_csv('death_cases.csv', sep=',')
df = df.T

date = str(start_month)+'/'+str(start_day)+'/'+ str(start_year)[2:]
date_index = df.index.get_loc(date)

reported_data = np.squeeze(df.iloc[date_index:date_index+days+1].values)

print(len(reported_data))

reported_data = reported_data + np.ones(len(reported_data))*1e-5
...
def neg_log_likelihood_logParam(theta, measurement = reported_data):
    sigma = theta[0]
...
ax = visualize.waterfall(result1_tnc, size=(15, 6))
# ax[0].title.set_text('BFGS Waterfall')
ax.title.set_text('TNC Waterfall')
...
tick_range[-1] -= 1
fig, ax = plt.subplots()
ax.plot(simulation, 'o-', markersize = 3, label='simulation') 
ax.plot(reported_data, 'o-', markersize = 3, label='measurement')
